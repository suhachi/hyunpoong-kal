  ...
  return (
    <div className="bg-white rounded-2xl p-6 space-y-4">
      ...
      <div className="space-y-3">
        ...
        <InfoRow label="주소" value="경남 양산시 물금읍 범어리 2699-9 202호" copyable />
        <InfoRow label="연락처" value="010-2068-4732" linkable="tel:010-2068-4732" />
      </div>
    </div>
  );
}
...
function InfoRow({ label, value, copyable, linkable }: InfoRowProps) {
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      <span className="text-sm text-[#2E1C10]/60 whitespace-nowrap">{label}</span>
      <div className="flex items-center gap-2">
        {linkable ? (
          <a href={linkable} className="text-sm text-[#D61C1C] hover:underline">
            {value}
          </a>
        ) : (
          <span className="text-sm text-[#2E1C10] text-right">{value}</span>
        )}
        {copyable && (
          <button
            onClick={handleCopy}

...
export async function fetchOrders(
  ...
  if (!USE_FIREBASE) {
    ...
    const getCreatedAtSeconds = (value: unknown): number => {
      if (!value) return 0;
  ...
  try {
    let q = query(collection(db, "orders"), where("storeId", "==", storeId));
    ...
    if (sortField === "amount") {
      orders.sort((a, b) =>
        sortDirection === "desc" ? b.finalAmount - a.finalAmount : a.finalAmount - b.finalAmount,
      );
    }
...
export async function updateOrderStatus(
  ...
  if (!USE_FIREBASE) {
    ...
    mockLogs.push({
      logId: `log-${Date.now()}`,
      orderId,
      action: newStatus === OrderStatus.CANCELLED ? "canceled" : "status_changed",
...
export async function fetchOrderStats(storeId: string): Promise<OrderStats> {
  ...
  try {
    ...
    return {
      ...
      pending: 0,
      accepted: 0,
      cooking: 0, // Changed from preparing to cooking to match OrderStats
      completed: 0,
      cancelled: 0, // Changed from canceled to cancelled
      todayRevenue: 0,
      todayOrders: 0,
    };
  }

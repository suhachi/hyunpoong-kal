 ...
 * 옵션 그룹 생성
 */
export async function createOptionGroup(data: Omit<OptionGroup, "id">): Promise<OptionGroup> {
  if (USE_FIREBASE) {
    // TODO: Firestore 연동
    throw new Error("Firebase not configured");
  }
 ...
 * 옵션 항목 삭제
 */
export async function deleteOptionItem(groupId: string, itemId: string): Promise<OptionGroup> {
  if (USE_FIREBASE) {
    // TODO: Firestore 연동
    throw new Error("Firebase not configured");
  }

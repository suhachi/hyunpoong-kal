...
test.describe.skip("Order flow (Mock, payment OFF)", () => {
  ...
  test("고객 주문이 관리자 주문 목록에도 표시된다 (Mock, payment OFF)", async ({
    page,
    browser,
  }) => {
    // ============ 고객: 주문 생성 ============
    ...
    // 헤더/테이블 표시 확인
    await expect(adminPage.getByText(/주문 관리|주문 현황|Orders/i)).toBeVisible({
      timeout: 10000,
    });
...
test.describe("@orderflow Firebase Order flow", () => {
  test("@orderflow 고객 주문이 완료되고 관리자에서 조회된다 (Firebase)", async ({
    page,
    browser,
  }) => {
    // ============================================================
    ...
    console.log(
      "[debug] pathname immediately after click:",
    ...
    if (!(await page.getByTestId("menu-detail.page").isVisible())) {
      ...
      if (href) {
        ...
        console.log(
          "[debug] performed fallback goto, pathname:",
          await page.evaluate(() => window.location.pathname),
        );
      }
    }
    ...
    await expect(adminPage.getByTestId("admin.orders.item.summary").first()).toBeVisible({
      timeout: 20000,
    });
    if (orderId) {
      ...
      await expect
        .poll(
          async () => {
            return await adminPage.evaluate(oid => {
              return !!Array.from(
                document.querySelectorAll('[data-testid="admin.orders.item.summary"]'),
        ...
        .toBeTruthy();
      await expect(adminPage.getByTestId("admin.orders.item.detail-button").first()).toBeVisible({
        timeout: 20000,
      });

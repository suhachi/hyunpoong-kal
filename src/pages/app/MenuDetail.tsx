...
export function MenuDetail() {
  ...
  const totalPrice = useMemo(() => {
    if (!menu) return 0;
    return (menu.price + optionPrices.noodle + optionPrices.toppings + optionPrices.custom) * quantity;
  ...
  const handleAddToCart = () => {
    ...
    // 선택된 커스텀 옵션 목록 생성
    const selectedCustomOptionsList = menu.customOptions?.filter(
      option => selectedCustomOptions[option.id]
    ) || [];
              ...
              {menu.customOptions.map((option) => (
                <div key={option.id} className="flex items-center space-x-2">
                  ...
                  <Label htmlFor={`custom-${option.id}`} className="flex-1 cursor-pointer">
                    ...
                    {option.quantity > 1 && (
                      <span className="text-[#2E1C10]/60 text-sm ml-1">
                        (x{option.quantity})
                      </span>
                    )}
                  </Label>

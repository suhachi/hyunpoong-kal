...
export async function generateReceiptPDF(data: ReceiptData): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // 헤더
      doc.fontSize(20).font("Helvetica-Bold").text("현풍닭칼국수 영수증", { align: "center" });
      ...
      // 결제수단
      doc.fontSize(10).font("Helvetica").text(`결제수단: ${data.paymentMethod}`);
      ...
      doc
        ...
        .text(`${data.developerInfo.company} | 사업자번호: ${data.developerInfo.bizNo}`, {
          align: "center",
        })

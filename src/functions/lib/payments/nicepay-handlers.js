...
async function createPaymentHandler(data, context) {
    ...
    try {
        ...
        const signData = await generateHash(`${NICEPAY_CONFIG.mid}${amount}${orderId}${ediDate}${NICEPAY_CONFIG.secretKey}`);
        ...
        throw new functions.https.HttpsError('internal', error.message || '결제 요청에 실패했습니다');
    }
}
/**
 * approvePayment 핸들러
...
async function getPaymentResultHandler(data, context) {
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', error.message || '결제 결과 조회에 실패했습니다');
    }
}
/**
 * cancelPayment 핸들러
...
async function cancelPaymentHandler(data, context) {
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', error.message || '결제 취소에 실패했습니다');
    }
}
/**
...
async function createOnSitePaymentOrderHandler(data, context) {
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', error.message || '주문 생성에 실패했습니다');
    }
}
